...
└── Secret Files/                # OAuth credentials + TLS certs (gitignored)
    ├── developer_credentials.json  # Google Drive OAuth2 client credentials
    ├── developer_token.json        # Google Drive OAuth2 token (generated by pre-auth step)
...
# Complete the browser OAuth flow — saves Secret Files/developer_token.json
```

Verify `Secret Files/developer_token.json` exists before building the Docker image.
