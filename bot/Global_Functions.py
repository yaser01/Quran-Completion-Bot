...
from db.models import Khatma_Parts
from domain.TelegramUser import TelegramUser
from config.Global_Files import Daily_Page_Quran_File
from config.UserOptionsKeys import UserOptionsKeys
...
def calc_next_notification_time(deadline_time: datetime) -> Optional[datetime]:
...
def build_async_db_uri(uri: str) -> str:
    if "+asyncpg" not in uri:
        return '+asyncpg:'.join(uri.split(":", 1))
    return uri


def format_timespan_in_arabic(total_time: float, max_units: int) -> str:
    formatted_time = ""
...
def create_keyboard(keyboard_list: list, one_time_keyboard: bool = False) -> ReplyKeyboardMarkup:
    return ReplyKeyboardMarkup(keyboard_list, resize_keyboard=True, one_time_keyboard=one_time_keyboard)


def get_text_copyable(text: str) -> str:
    return "`" + str(text) + "`"


def get_elided_text_version(text: str, limit: int = 20) -> str:
    new_text = text[:limit]
...
async def send_missing_data_message(telegram_user: TelegramUser, context) -> None:
...
def get_user_identity_string(user: Any) -> str:
...
def get_user_identity_string_from_telegram_user_object(user: TelegramUser) -> str:
    user_identity_string = "User(ID: " + str(user.id) + ", " + "Username: " + str(
        user.username) + ", " + "Fullname: " + str(user.fullname) + "): "
    return user_identity_string


def compare_between_part_asc(item1: Khatma_Parts, item2: Khatma_Parts) -> int:
...
def compare_between_part_desc(item1: Khatma_Parts, item2: Khatma_Parts) -> int:
    if item1.Khatma.id != item2.Khatma.id:
...
async def delete_old_khatma_opening_request(chat_id: int, context) -> None:
    if "new_khatma_message_id" in context.user_data.keys():
        try:
            ...
            logging.debug(f"Failed to delete old khatma opening message: {e}")
    if "khatma_info_request_message_id" in context.user_data.keys():
        try:
            ...
            logging.debug(f"Failed to delete old khatma info request message: {e}")


def get_local_date_from_utc_time(utc_time: datetime) -> str:
    from_zone = pytz.utc
...
def set_next_page_no_daily_page_quran() -> None:
    with open(Daily_Page_Quran_File, "r+", encoding="UTF-8") as f:
        data = json.load(f)
