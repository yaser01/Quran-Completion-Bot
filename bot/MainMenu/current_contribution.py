...
from db import db
from bot.Brodcast import send_completed_message_to_users
from bot.Global_Functions import delete_old_khatma_opening_request, compare_between_part_asc, \
...
async def current_contribution_parts_pressed(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        logging.debug(f"Failed to edit current contributions message: {e}")
        await query.answer()
...
async def options_khatma_part_pressed(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        logging.debug(f"Failed to edit khatma part options message: {e}")
...
async def mark_part_as_cancel_pressed(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        logging.debug(f"Failed to edit contributions list after cancel: {e}")
...
async def mark_part_as_done_pressed(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        logging.debug(f"Failed to edit contributions list after done: {e}")
