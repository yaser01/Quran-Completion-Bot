...
from db import db
from bot.Global_Functions import delete_old_khatma_opening_request, clear_user_data
...
@register_state(UserStates.Contribute_Private_Khatma_Waiting_ID)
async def on_contribute_private_khatma_waiting_id(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    chat_id = update.message.from_user.id
    khatma_id = update.message.text
    back_main_menu_keyboard = InlineKeyboardMarkup(Keyboards.get_back_to_main_menu())
    if not khatma_id.isdigit() or int(khatma_id) <= 0:
        logging.warning(f"Invalid private khatma ID entered: {khatma_id!r}")
        msg = await context.bot.send_message(chat_id=chat_id,
                                             reply_markup=back_main_menu_keyboard,
                                             text=Text.Please_Enter_Correct_Number)
        context.user_data["khatma_private_id_request_message_id"] = msg.id
        return
    if "temp_message_id" in context.user_data:
        try:
            await context.bot.delete_message(chat_id=chat_id, message_id=context.user_data.pop("temp_message_id"))
        except Exception as e:
            logging.debug(f"Failed to delete temp message: {e}")
    ...
    if khatma_status == KhatmaStatus.Opened:
        text = Text.create_khatma_info(khatma_id=khatma_id, name=khatma_data.name_of_opener,
                                       intention=khatma_data.description,
                                       duration_in_days=khatma_data.number_of_days_to_finish_a_part,
                                       start_date=khatma_data.time)
        keyboard = InlineKeyboardMarkup(Keyboards.get_khatma_parts_keyboard(khatma_id=khatma_id,
                                                                            khatma_parts=khatma_parts_data))
    elif khatma_status == KhatmaStatus.Done:
        text = Text.create_khatma_info(khatma_id=khatma_id, name=khatma_data.name_of_opener,
                                       intention=khatma_data.description, duration_in_days=0,
                                       start_date=khatma_data.start_date, end_date=khatma_data.end_date,
                                       is_finished=True, is_canceled=False)
        keyboard = InlineKeyboardMarkup(Keyboards.get_back_to_main_menu())
    else:
        text = Text.create_khatma_info(khatma_id=khatma_id, name=khatma_data.name_of_opener,
                                       intention=khatma_data.description, duration_in_days=0,
                                       start_date=khatma_data.start_date, end_date=khatma_data.end_date,
                                       is_finished=False, is_canceled=True)
        keyboard = InlineKeyboardMarkup(Keyboards.get_back_to_main_menu())
    await context.bot.send_message(chat_id=chat_id, reply_markup=keyboard, text=text,
...
async def contribute_to_public_khatma_options_pressed(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        logging.debug(f"Failed to edit message: {e}")
