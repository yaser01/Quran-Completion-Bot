...
from db import db
from bot.Global_Functions import QURAN_BOOK_ID
...
@register_state(UserStates.Browse_Quran_Waiting_For_Page_No)
async def on_browse_quran_waiting_page_no(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    chat_id = update.message.from_user.id
    page_no_text = update.message.text
    if not page_no_text.isdigit():
        logging.warning(f"Invalid Quran page number entered: {page_no_text!r}")
        await context.bot.send_message(chat_id=chat_id, text=Text.Wrong_Input_Browse_Quran)
        return
    page_no = int(page_no_text)
    if page_no < 1 or page_no > 604:
        logging.warning(f"Quran page number out of range: {page_no}")
        await context.bot.send_message(chat_id=chat_id, text=Text.Wrong_Range_Browse_Quran)
        return
...
async def send_quran_chapter_file_pressed(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        logging.debug(f"Failed to send document: {e}")
