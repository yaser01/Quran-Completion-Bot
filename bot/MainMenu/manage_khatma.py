...
from db import db
from bot.Brodcast import send_completed_message_to_users, send_canceled_khatma_message_to_users
from bot.Global_Functions import send_missing_data_message, get_user_object_date_from_update_object
...
@register_state(UserStates.Update_Khatma_Waiting_For_Opener_Name)
async def on_update_khatma_waiting_opener_name(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    chat_id = update.message.from_user.id
    telegram_user = get_user_object_date_from_update_object(update)
    opener_name = update.message.text
    if "khatma_id" not in context.user_data:
        logging.warning("manage khatma flow: khatma_id missing from user_data")
        await send_missing_data_message(telegram_user=telegram_user, context=context)
        return
    khatma_id = int(context.user_data.pop("khatma_id"))
    result = await db.update_khatma_opener_name(khatma_id, new_opener_name=opener_name)
    await context.bot.send_message(
        chat_id=chat_id,
        text=Text.Update_Khatma_Info_Done if result else Text.Unknown_Error_Text)
    context.user_data["state"] = UserStates.Nothing
    try:
        if context.user_data.get("temp_message_id") is not None:
            await context.bot.delete_message(chat_id=chat_id,
                                             message_id=context.user_data.pop("temp_message_id"))
    except Exception as e:
        logging.debug(f"Failed to delete temp message after opener name update: {e}")


@register_state(UserStates.Update_Khatma_Waiting_For_Intention)
async def on_update_khatma_waiting_intention(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    chat_id = update.message.from_user.id
    telegram_user = get_user_object_date_from_update_object(update)
    ...
    try:
        ...
        logging.debug(f"Failed to delete temp message after intention update: {e}")


@register_state(UserStates.Update_Khatma_Waiting_For_Part_Duration)
async def on_update_khatma_waiting_duration(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    chat_id = update.message.from_user.id
    telegram_user = get_user_object_date_from_update_object(update)
    ...
    if not duration_text.isdigit() or int(duration_text) <= 0:
        logging.warning(f"Invalid update duration entered: {duration_text!r}")
        await context.bot.send_message(chat_id=chat_id, text=Text.Please_Enter_Correct_Number)
        return
    if int(duration_text) > 60:
        logging.warning(f"Update duration too large: {duration_text!r}")
        await context.bot.send_message(
            chat_id=chat_id,
    ...
    result = await db.update_khatma_reservation_part_duration(khatma_id,
                                                              new_reservation_part_duration=int(duration_text))
    await context.bot.send_message(
        chat_id=chat_id,
        text=Text.Update_Khatma_Info_Done if result else Text.Unknown_Error_Text)
    context.user_data["state"] = UserStates.Nothing
    try:
        ...
        logging.debug(f"Failed to delete temp message after duration update: {e}")
...
async def manage_khatma_pressed(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        logging.debug(f"Failed to edit manage khatma message: {e}")
...
async def manage_khatma_properties_pressed(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        logging.debug(f"Failed to edit manage khatma properties message: {e}")
...
async def manage_khatma_update_name_pressed(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        logging.warning(f"Failed to send update-name prompt: {e}")
...
async def manage_khatma_update_intention_pressed(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        logging.warning(f"Failed to send update-intention prompt: {e}")
...
async def manage_khatma_update_part_duration_pressed(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        logging.warning(f"Failed to send update-duration prompt: {e}")
...
async def manage_khatma_update_type_pressed(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        logging.warning(f"Failed to send update-type prompt: {e}")
...
async def manage_khatma_parts_options_pressed(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        logging.debug(f"Failed to edit khatma parts options message: {e}")
...
async def mark_part_as_occupied_by_admin_pressed(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        logging.debug(f"Failed to edit message after admin part book: {e}")
...
async def mark_part_as_cancel_occupied_by_admin_pressed(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        logging.debug(f"Failed to edit message after admin part cancel: {e}")

    try:
        await context.bot.send_message(chat_id=user_id, text=message_text_for_user)
    except Exception as e:
        logging.warning(f"Failed to notify user {user_id} of part cancellation: {e}")
...
async def mark_part_as_done_by_admin_pressed(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        logging.debug(f"Failed to edit message after admin part done: {e}")

    try:
        await context.bot.send_message(chat_id=user_id, text=message_text_for_user)
    except Exception as e:
        logging.warning(f"Failed to notify user {user_id} of part completion by admin: {e}")
...
async def mark_part_as_cancel_read_by_admin_pressed(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        logging.debug(f"Failed to edit message after admin cancel-read: {e}")

    try:
        await context.bot.send_message(chat_id=user_id, text=message_text_for_user)
    except Exception as e:
        logging.warning(f"Failed to notify user {user_id} of part cancel-read by admin: {e}")
...
async def mark_khatma_as_cancel_by_admin_pressed(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        logging.debug(f"Failed to edit cancel-khatma confirmation message: {e}")
...
async def mark_khatma_as_cancel_by_admin_confirmed_pressed(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        logging.debug(f"Failed to edit message after khatma cancel confirmed: {e}")
...
async def mark_khatma_as_done_by_admin_pressed(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        logging.debug(f"Failed to edit done-khatma confirmation message: {e}")
...
async def mark_khatma_as_done_by_admin_confirmed_pressed(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        logging.debug(f"Failed to edit message after khatma done confirmed: {e}")
